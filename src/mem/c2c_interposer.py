...
class C2CInterposer(ClockedObject):
    ...
    # NUEVO:
    # Antes el interposer tenia 4 puertos fijos:
    #   from_interface0_port, to_interface0_port,
    #   from_interface1_port, to_interface1_port.
    # Eso solo sirve para una conexion punto a punto entre 2 chips.
    # Ahora lo convertimos en un MUX C2C con puertos vectoriales:
    #   - from_interfaces[i]: puerto ResponsePort por donde entra trafico desde el chip i.
    #   - to_interfaces[i]:   puerto RequestPort por donde sale trafico hacia el chip i.
    # Asi se pueden conectar 3 chips, o mas, a una unica entidad central.
    ...
    # Numero de chips/interfaces conectadas al MUX.
    ...
    # Latencia que el MUX anade a mensajes que viajan por el canal de request/snoop.
    req_latency = Param.Cycles(1, "Latency added to C2C request/snoop traffic")

    # Latencia que el MUX anade a mensajes que viajan por el canal de response/data.
    resp_latency = Param.Cycles(1, "Latency added to C2C response/data traffic")

    # Tamano maximo de cada cola por clase logica y por chip destino.
    # No es un unico buffer global: hay una cola por destino y por clase
    ...
    # Rangos de memoria por chip. El indice de la lista es el chipID.
    ...
    # Es el mismo cacheChipIDList que ya estabas construyendo en la jerarquia.
    ...
    # Normalmente, para 3 chips sera [0, 1, 2].
